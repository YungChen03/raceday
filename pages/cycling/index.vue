  ...
  <div class="lime lighten-5">
    <v-container class="my-3 pa-5">
      <v-layout row class="ml-2" align-start>
        ...
        <!-- <v-btn text mr-5 color="grey darken-2" @click="showSearchBar">
          <v-icon left>mdi-magnify</v-icon>
          <span class="caption text-lowercase">search event</span>
        </v-btn>
        <input
          @keydown.enter="searchby(searchValue)"
          mt-10
          v-show="isActive"
...
export default {
  ...
  methods: {
    ...
    goDetail(id) {
      this.$router.push(`/cycling/${id}`);
    },
    searchby(value) {
      return this.formattedEvent.filter(title => {
  ...
  mounted() {
    const load = async () => {
      try {
        let res = await this.$getdocs.getData("cycling");
        this.events = res;
      } catch (err) {
